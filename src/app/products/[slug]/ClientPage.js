...
export default function ProductDetailPage({ params }) {
    ...
    fetch(`/api/products/${slug}`)
      ...
      .then((d) => {
        ...
        if (d.product?.sizes?.length)
          setSelectedSize(d.product.sizes[1] || d.product.sizes[0]);
        if (d.product?.colors?.length)
  ...
  const isMidnightBlackColor =
    selectedColor?.toLowerCase() === "midnight black";

  let dynamicFilterStyle = {};
  if (isOliveColor) {
    dynamicFilterStyle = {
      filter:
        "sepia(0.55) hue-rotate(65deg) saturate(1.15) brightness(0.85) contrast(1.05)",
    };
  }

  const getLensFilter = () => {
    if (isOliveColor)
      return "sepia(0.55) hue-rotate(65deg) saturate(1.15) brightness(0.85) contrast(1.05)";
    return "none";
  };
      ...
      <div className="pd-layout">
        <div className="pd-gallery">
            ...
            {getProductImages().map((img, i) => (
              ...
              >
                <Image
                  ...
                  fill
                  unoptimized={true}
                  style={{ objectFit: "cover", ...dynamicFilterStyle }}
                />
              </div>
            ))}
          </div>
          ...
          >
            {getProductImages()[mainImage] && (
              <Image
                src={getProductImages()[mainImage]}
                alt={product.name}
                fill
                unoptimized={true}
                style={{ objectFit: "cover", ...dynamicFilterStyle }}
              ...
              <>
                <div
                  style={{
                    ...
                    pointerEvents: "none",
                    boxShadow:
                      "0 20px 50px rgba(26, 20, 16, 0.35), inset 0 0 25px rgba(255, 255, 255, 0.25)",
                    ...
                    transition:
                      "left 0.12s cubic-bezier(0.215, 0.610, 0.355, 1.000), top 0.12s cubic-bezier(0.215, 0.610, 0.355, 1.000), background-position 0.12s cubic-bezier(0.215, 0.610, 0.355, 1.000)",
                  }}
                >
        ...
        <div className="pd-info">
          {product.collection_name && (
            <p className="pd-collection">{product.collection_name}</p>
          )}
          <h1>{product.name}</h1>
          ...
          {avgRating > 0 && (
            ...
            >
              {stars(avgRating)}{" "}
              <span style={{ color: "var(--taupe)", fontSize: ".72rem" }}>
              ...
              <div className="pd-colors">
                {product.colors.map((c, i) => (
                  <div
                    ...
                    className={`pd-color ${selectedColor === c.name ? "active" : ""}`}
                    onClick={() => {
          ...
          <div className="pd-actions">
            <div
              ...
              style={{
                border: "1px solid rgba(158,139,124,.3)",
                padding: ".2rem",
              }}
            >
          ...
          <div className="pd-accordion">
            {[
              ...
              },
              {
                ...
                content:
                  product.fit ||
                  "Please refer to our sizing guide for detailed measurements.",
              },
              {
                key: "care",
                ...
                content:
                  product.care ||
                  "Please follow the care label inside the garment.",
              },
              {
                key: "shipping",
                label: "Shipping",
                content:
                  "Complimentary shipping on orders over ₹4,999. Standard delivery: 5-7 business days. Express delivery: 2-3 business days (₹299).",
              },
            ].map((a) => (
              <div className="pd-accordion-item" key={a.key}>
                <button
                  ...
                  onClick={() =>
                    setOpenAccordion(openAccordion === a.key ? "" : a.key)
                  }
                >
                  {a.label} <span>{openAccordion === a.key ? "−" : "+"}</span>
                </button>
                <div
                  className={`pd-accordion-body ${openAccordion === a.key ? "open" : ""}`}
                >
                  {a.content}
                </div>
              </div>
          ...
          {reviews.length > 0 ? (
            ...
            >
              {reviews.map((r) => (
                ...
                >
                  <div style={{ color: "var(--rose)", marginBottom: ".5rem" }}>
                    {stars(r.rating)}
                  </div>
                  <p
                    style={{
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: ".75rem", color: "var(--stone)" }}>
              ₹{product.price.toLocaleString("en-IN")}
            </p>
          </div>
        </div>
          ...
          <>
            ...
            >
              ...
              <div className="spec-body">
                ...
                {[
                  {
                    name: "Thermal Breathability",
                    val: product.weaveSpecs.breathability,
                  },
                  { name: "Drape Fluidity", val: product.weaveSpecs.drape },
                  {
                    name: "Tactile Softness",
                    val: product.weaveSpecs.softness,
                  },
                ].map((bar) => (

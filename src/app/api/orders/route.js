...
export async function GET(request) {
  ...
  const db = getDb();
  const orders = db.prepare('SELECT * FROM orders WHERE user_id = ? ORDER BY created_at DESC').all(payload.id);
...
export async function POST(request) {
  try {
    ...
    const placeOrderTransaction = db.transaction((payloadId, shippingData, customerEmail, paymentMethod, cartItems, promoCode) => {
      let subtotal = 0;
      const enrichedItems = [];

      const getProduct = db.prepare('SELECT * FROM products WHERE id = ?');
      const updateStock = db.prepare('UPDATE products SET in_stock = MAX(0, in_stock - ?) WHERE id = ?');
      ...
      const finalPaymentMethod = promoCode ? `${paymentMethod} (${promoCode})` : paymentMethod;
      ...
      `).run(
        payloadId, finalSubtotal, shippingCost, total,
        shippingData?.name || '', customerEmail || shippingData?.email || '', shippingData?.phone || '',
        shippingData?.address || '', shippingData?.city || '', shippingData?.state || '',
        shippingData?.pincode || '', finalPaymentMethod || 'card'
      );
      ...
      for (const enriched of enrichedItems) {
        insertItem.run(
          orderId, enriched.product_id, enriched.name, enriched.image, 
          enriched.size || '', enriched.color || '', enriched.quantity, enriched.price
        );
      }
